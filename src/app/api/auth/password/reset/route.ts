...
export const POST = withApiObservability(async (req: Request) => {
  ...
  if (!row) return fail({ status: 400, code: "TOKEN_INVALID" })
  if (row.usedAt) return fail({ status: 400, code: "TOKEN_USED" })
  if (row.expiresAt.getTime() <= now.getTime()) return fail({ status: 400, code: "TOKEN_EXPIRED" })
