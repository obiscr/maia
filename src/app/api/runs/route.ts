...
export const GET = withApiObservability(async (req: Request) => {
  ...
  function parseInputCounts(initialInputRaw: string | null | undefined) {
  ...
  // Derive lightweight per-run meta for list display: step progress + "failed/running step name".
  // Important: still keep this bounded and avoid returning full steps/attempts payloads.
