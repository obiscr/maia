...
import { generateInputSpec } from "@/lib/server/chat/tools"
...
export const POST = withApiObservability(async (req: Request, ctx: { params: Promise<{ workflowId: string }> }) => {
  ...
  const inputSpec = await generateInputSpec({ draft, locale: body.locale, model })

  if (!inputSpec) return fail({ status: 500, code: "INPUT_SPEC_GENERATION_FAILED" })
