...
export function useRunDetailBottomPanelData(args: {
  ...
  const inputFiles: InputFileRow[] = Array.isArray(runInputFilesQuery.data)
    ? (runInputFilesQuery.data as InputFileRow[])
    : []
  ...
  const fileViewerFiles = (inputFiles ?? []).map((f) =>
    inputFileToFileViewerModel(args.runId, f, args.fileFallbackName),
  )
