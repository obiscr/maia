...
// Note:
// - These are feature prompts, NOT UI copy; keep them out of i18n JSON.
// - Do not hard-code step counts; let the agent decide structure/size.
// - Ask for a clear plan first, then publish draft steps, then validate.

const ZH_PROMPTS: Record<WorkflowExampleId, string> = {
  ex1: `帮我做一个工作流：我给你一个网页链接，你帮我把网页标题、简介（有就取，没有就空）、再写一段短摘要，最后输出成一个 JSON。
...
另外把 inputSpec v1 也一起写好（required 尽量少，至少给 2 个 examples）。步骤脚本用系统要求的 export default async main(...) 结构。

最后：先给一个清晰 plan，再逐步 publish_draft_step，最后 validate_workflow_payload。`,
...
把 inputSpec v1 写好（required 少一点，给 2 个 examples）。

最后：先 plan，再逐步 publish_draft_step，最后 validate_workflow_payload。`,

  ex3: `我这边会传一段 JSON（params.payload），但数据可能不太干净。你帮我做个工作流：先检查一下格式/字段对不对 → 再把字段规范化（比如补默认值、改字段名、类型转换）→ 最后输出一个“干净版”的 JSON（outputs.result），最好还带点校验信息。

如果校验失败，不要只给一句话：请输出结构化 errors（字段路径、原因、怎么修）。
inputSpec v1 也一起生成，required 尽量少，给 examples。

最后：先 plan → publish_draft_step → validate_workflow_payload。`,

  ex4: `给你一个 RSS/Atom 链接（params.feedUrl），帮我做个工作流：抓取 feed → 把条目整理干净（标题、链接、时间、来源）→ 去重一下 → 输出一个 items 列表（默认拿一小部分就行，maxItems 作为可选参数）。
...
顺便把 inputSpec v1 写好。`,
  ...
  ex6: `我想把一段视频/会议的字幕整理成“要点 + 待办”。你帮我做个工作流：
...
inputSpec v1 要支持“只有 transcript 也能跑”；失败要有降级。`,
...
inputSpec v1 也写好，最后一个最终输出步骤。`,
  ...
  ex11: `我想做一个“Issue 分诊小帮手”。输入就是标题（params.title）、正文（params.body），评论（params.comments）可选。请输出一个 triage 结果：建议的 labels、优先级（带理由）、复现步骤提取、以及一段可以直接粘贴回去的建议回复。
...
inputSpec v1 required 少一点，最后一步输出。`,
  ...
  ex13: `我有好几个信息源（params.sources，一堆 URL，可能是 RSS 也可能是网页），想把它们合成一个“去重后的摘要”。请做个工作流：抓取 → 去重 → 按主题分组/聚类 → 输出 clusters + digest。

抓取要考虑限速、重试、降级（比如某个源挂了就跳过但记录下来），diagnostics 里要看得出来发生了什么。最后一步输出。`,

  ex14: `我想把一次 A/B 实验的结果做成一份“能看懂的分析报告”。输入是对照组/实验组的指标和样本量（CSV 或 JSON 都行）。你做个工作流：算提升幅度、给置信区间/显著性，并用人话解释结论和注意事项；同时做一些基本检查（比如 SRM、缺失数据）。

输出 report + checks 两块 JSON；inputSpec v1 + examples；最后一步输出。`,

  ex15: `我有一堆客服工单（CSV 或 JSON），想让你帮我做个“自动分流 + 风险提示”的工作流：先把工单大概分个类 → 建议分给哪个团队 → 估一下 SLA 风险（高/中/低 + 理由）→ 抽取里面的关键信息（订单号等），但输出里要把邮箱/手机号这类 PII 脱敏。

最后输出一个 queuePlan（每条工单的路由/优先级/风险/摘要）和 redaction（脱敏命中统计）。inputSpec v1 写好，最后一步输出。`,
...
- inputSpec v1（含 examples）要给出来；
- 允许并行处理，但最后一定要汇聚；
- 分块 ID 可复现，抓取/解析失败要有重试与降级，并在 diagnostics 里能看出来；
- step 脚本结构与 ctx 读取规则严格遵守系统提示。

最后：先给 plan，再逐步 publish_draft_step，最后 validate_workflow_payload。`,
...
要求：把 inputSpec v1 写好；要有可重试/可降级；依赖清晰。`,

  ex18: `我有一堆网站链接（params.urls），想做个“站点变更监控”的工作流：定期抓一下页面，看看内容有没有变化；如果变化比较大就出告警，变化不大就做个汇总。
...
重点：抓取要考虑限速、缓存、按域名退避（别把别人网站打挂）。inputSpec v1 + examples。`,

  ex19: `我要你为 Maia 设计一个“混合来源知识库构建：采集 → 规范化 → 去重 → 引用/出处抽取 → 索引构建 → QA 生成 → 一致性审计”的复杂工作流，并输出可执行 workflow draft。
...
要求：inputSpec v1 要有；每条事实必须带 provenance（能追溯来源）。`,

  ex20: `我要你为 Maia 设计一个“多源情报采集 → 清洗归一 → 实体抽取 → 去重聚合 → 风险评分 → 报告生成 → 通知分发”的超复杂工作流，并输出可执行的 workflow draft（name/description/dependencies/envJson/inputSpec/steps）。
...
最后：请先给清晰 plan，再按计划逐步 publish_draft_step（每步一次 tool call），最后 validate_workflow_payload 输出完整 draft。`,
}

const EN_PROMPTS: Record<WorkflowExampleId, string> = {
  ex1: `Help me build a workflow: I’ll give you a web URL, and I want a clean JSON output with the page title, a short description (if available), and a short summary.
...
Also generate inputSpec v1 (keep required fields minimal, include at least 2 examples). Each step script must follow the system’s export default async main(...) format.

Finally: give a clear plan first, then publish_draft_step step-by-step, then validate_workflow_payload with the full draft.`,
...
Please don’t blow up on large files — sampling/chunking is fine, just explain what you did and record errors when needed. Generate inputSpec v1 (minimal required fields + 2 examples).`,
  ...
  ex3: `I’ll send you a JSON payload (params.payload) but it can be messy. Please build a workflow that validates it, normalizes fields (defaults, renames, type coercion), derives a few useful fields, and outputs a “clean” JSON result.

If validation fails, don’t just say “invalid” — return structured errors (path, reason, suggested fix). Also provide inputSpec v1 with minimal required fields + examples.`,

  ex4: `Build me a workflow that takes an RSS/Atom URL (params.feedUrl), fetches it, cleans up items (title/link/date/source), dedupes them, and outputs a small digest list (maxItems can be an optional parameter).
...
If OCR quality is poor, still output the raw OCR text as a fallback. Provide inputSpec v1 + examples.`,
  ...
  ex6: `I want to turn a meeting/video transcript into “key takeaways + action items”. Build a workflow where I can either provide a video URL (params.videoUrl) or just paste the transcript (params.transcript).
...
Provide inputSpec v1 + examples, handle extraction failures gracefully.`,
  ...
  ex11: `Build me an “issue triage helper”. Input is title/body (comments optional). Output should include suggested labels, a priority (with rationale), extracted repro steps, and a short reply message I can paste back.

Keep it practical (like a real support/engineering team would do). Provide inputSpec v1 + examples.`,

  ex12: `I only have sample data (CSV/JSON). Build a workflow that infers a schema, proposes constraints (required/ranges/enums), flags possible PII columns, and outputs JSON Schema plus recommendations/assumptions.

Keep required fields minimal in inputSpec v1, include examples.`,

  ex13: `I have multiple sources (a list of URLs, can be RSS or web pages) and I want a merged digest: fetch, dedupe, cluster by topic, and output clusters + a clean digest.

Please be explicit about rate limits, retries, and fallback behavior, and put diagnostics in the output. Provide inputSpec v1 + examples.`,

  ex14: `Turn my A/B experiment metrics into an analysis report people can understand. Input is control vs treatment metrics and sample sizes (CSV or JSON). Compute lift, confidence intervals/significance, and write a plain-language interpretation + caveats. Also run basic sanity checks (SRM, missing data).

Output report + checks JSON, provide inputSpec v1 + examples.`,

  ex15: `I have support tickets and I want a workflow that helps triage them: classify intent, suggest routing to a team, estimate SLA risk (high/med/low + why), extract useful entities (order id, etc.), and redact PII in outputs.

Output a queuePlan plus redaction stats, generate inputSpec v1 + examples, and ensure we don’t leak PII.`,
...
Constraints: inputSpec v1; clear deps; retries + degradation.`,

  ex18: `Design a complex Maia workflow for “multi-lingual site monitoring: fetch → change detection → change classification → optional translation → alert generation → notification”, and output an executable workflow draft.
...
Constraints: must handle caching, rate limits, and per-domain backoff; inputSpec v1.`,

  ex19: `Design a complex Maia workflow for “knowledge base building from mixed sources: ingest → normalize → dedupe → provenance extraction → indexing → QA generation → consistency audit”, and output an executable workflow draft.
...
Constraints: strict provenance; inputSpec v1.`,

  ex20: `Design an ultra-complex Maia workflow for “multi-source intel collection → normalization → entity extraction → dedupe/aggregation → risk scoring → report generation → notification”, and output an executable workflow draft (name/description/dependencies/envJson/inputSpec/steps).
...
Finally: provide a clear plan first, then publish_draft_step step-by-step, then validate_workflow_payload with the full draft.`,
}

export function getWorkflowExamplePrompt(locale: string | undefined | null, id: WorkflowExampleId): string {
  const l = normalizeLocale(locale)
  return (l === "zh-cn" ? ZH_PROMPTS : EN_PROMPTS)[id]
}
