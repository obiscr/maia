...
const ZH_WORKFLOW_TEMPLATES: Record<WorkflowTemplateId, string> = {
  web_summary: `帮我做一个工作流：我给你一个网页链接，你帮我把网页标题、简介（有就取，没有就空）、再写一段短摘要，最后输出成一个 JSON。
...
最后：先给一个清晰 plan，再逐步 draft_step，最后 finalize_draft。`,
  ...
  csv_stats: `我有一个 CSV 文件，想让你帮我做个"自动统计小助手"的工作流：读入 CSV → 识别每列是什么类型 → 统计一下（数值列算均值/最大最小/中位数等，文本列看 Top 值）→ 顺便做点数据质量提示（缺失、空值、异常）。
...
最后：先 plan，再逐步 draft_step，最后 finalize_draft。`,
  ...
  json_validate: `我这边会传一段 JSON（params.payload），但数据可能不太干净。你帮我做个工作流：先检查一下格式/字段对不对 → 再把字段规范化（比如补默认值、改字段名、类型转换）→ 最后输出一个"干净版"的 JSON（outputs.result），最好还带点校验信息。
...
最后：先 plan → draft_step → finalize_draft。`,
  ...
  rss_digest: `给你一个 RSS/Atom 链接（params.feedUrl），帮我做个工作流：抓取 feed → 把条目整理干净（标题、链接、时间、来源）→ 去重一下 → 输出一个 items 列表（默认拿一小部分就行，maxItems 作为可选参数）。

抓取要考虑失败/超时/无效 feed；把 errors 和你采取的降级策略写在 meta 里。
inputSpec v1 也一起写好。`,

  image_ocr: `我会给你一张图片（filesInput.image），你帮我把里面的文字识别出来（OCR），然后把文本清理一下（去多余空白、排版弄整齐），最后输出：text + 一些 meta（比如猜的语言、置信度、错误原因）。

如果 OCR 效果不佳也别崩：至少把"原始 OCR 文本"输出出来作为降级。
顺便把 inputSpec v1 写好。`,

  video_takeaways: `我想把一段视频/会议的字幕整理成"要点 + 待办"。你帮我做个工作流：
...
inputSpec v1 要支持"只有 transcript 也能跑"；失败要有降级。`,
  ...
  markdown_outline: `我有一份 Markdown 文档，想把它整理成结构化信息。请做个工作流：解析 Markdown → 生成大纲树（outline）→ 把代码块（含语言）和链接都抽出来 → 输出成 JSON。

输入支持文件（filesInput.markdown）或者直接传文本（params.markdownText）都行；保持原始顺序。`,

  log_errors: `我有一堆日志（filesInput.log），想快速知道最常见的报错是啥。请做个工作流：读日志 → 把错误按"同一类"聚起来 → 统计次数/首次出现/最后出现/给几行样例 → 输出一个报告 JSON（summary + groups）。topK 可以做成可选参数。

日志可能很大，别一次性全塞进内存：用采样/分块都行，但要说明，并把失败项写进 errors。`,

  review_sentiment: `我有一批商品/应用评论（text + rating），想看整体口碑、大家都在吐槽什么。请做个工作流：清洗评论 → 识别语言 → 判情绪倾向 → 找出常见主题 → 汇总成一个 dashboard JSON（按主题/情绪分布，顺便挑几条代表性评论）。

输入可以是 CSV 文件（filesInput.reviewsCsv）或直接传 JSON（params.reviewsJson）。请做重复/超短评论的质量检查，并在 diagnostics 里给出统计和 errors。`,

  invoice_parse: `我有一张发票 PDF（filesInput.invoicePdf），想把它变成结构化数据。请做个工作流：先把文字提取出来（必要时 OCR）→ 识别供应商/发票号/日期/总金额 → 把行项目抽出来 → 做个校验（比如行项目加起来跟总计对不对）→ 输出 invoice JSON + validation（告警要解释原因）。

inputSpec v1 也写好，最后一个最终输出步骤。`,

  issue_triage: `我想做一个"Issue 分诊小帮手"。输入就是标题（params.title）、正文（params.body），评论（params.comments）可选。请输出一个 triage 结果：建议的 labels、优先级（带理由）、复现步骤提取、以及一段可以直接粘贴回去的建议回复。

尽量像真实团队在做分诊那样写，不要太学术；inputSpec v1 + examples；最后一步输出。`,

  data_schema: `我手上只有一份"样本数据"（CSV 或 JSON，filesInput.sample），但我想快速知道这份数据大概长什么样、有哪些字段、应该怎么校验。请做个工作流：解析样本 → 推断 schema → 给约束建议（必填/范围/枚举）→ 顺便提示哪些列可能是 PII → 输出 JSON Schema + recommendations（把你的假设也写出来）。

inputSpec v1 required 少一点，最后一步输出。`,

  news_merge: `我有好几个信息源（params.sources，一堆 URL，可能是 RSS 也可能是网页），想把它们合成一个"去重后的摘要"。请做个工作流：抓取 → 去重 → 按主题分组/聚类 → 输出 clusters + digest。

抓取要考虑限速、重试、降级（比如某个源挂了就跳过但记录下来），diagnostics 里要看得出来发生了什么。最后一步输出。`,

  ab_analysis: `我想把一次 A/B 实验的结果做成一份"能看懂的分析报告"。输入是对照组/实验组的指标和样本量（CSV 或 JSON 都行）。你做个工作流：算提升幅度、给置信区间/显著性，并用人话解释结论和注意事项；同时做一些基本检查（比如 SRM、缺失数据）。
